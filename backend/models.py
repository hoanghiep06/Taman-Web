# models.py
import uuid
from sqlalchemy import Column, Integer, String, Boolean, Text, ForeignKey, Date, BigInteger
...
class Nonce(Base):
    ...
    # Sinh Nonce bằng UUID random
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))

    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)

    # Định danh thiết bị xin mã đối chiếu lúc upload ảnh
    ip_address = Column(String(45), nullable=False)
    ...
    # Trạng thái sử dụng (False Default, dùng xong thành True)
...
class InspectionLog(Base):
    ...
    # Liên kết log kiểm kê này với mã Nonce hợp lệ đã sử dụng để chống gian lận
...
class ShiftSummary(Base):
    ...
    # Lưu mảng ID để truy vết vĩnh viễn
...
# NHẬT KÝ HỆ THỐNG BẤT BIẾN (APPEND ONLY)
class AuditLog(Base):
    ...
    # Hành động "REQUEST_NONCE", "CHECKIN_SUCCESS", "ATTACK_DETECTED_WRONG_NONCE"
